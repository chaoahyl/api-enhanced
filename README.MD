...
| **AHYLO_BILIBILI_GATEWAY_TOKEN** | 无 | 启用 `/bilibili/*` 网关所需的 Bearer 密钥，必须与 `ahylo-api` 的 `BILIBILI_API_TOKEN` 相同。 |
| **BILIBILI_COOKIE**        | 无                                   | B 站网关可选登录 Cookie，只能通过部署环境变量配置，不能提交到仓库。            |
